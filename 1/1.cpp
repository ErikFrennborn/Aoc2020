...
int main()
{
    ...
    for (int i = 0; i < 200; i ++)
    {
        ...
        {
            for (int k = j+2; k < 200; k++)
            {
                if (input[i] + input[j] + input[k] == 2020)
                {
                    cout << input[i] << " " << input[j] << " " << input[k] << " " << input[i] * input[j] * input[k] << endl;
                    exit(EXIT_SUCCESS); 
                }
            }
        }
